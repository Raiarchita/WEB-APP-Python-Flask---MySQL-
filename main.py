...
app.config['MYSQL_PASSWORD'] = 'mtnl@2003'
...
def home():
    if 'loggedin' in session:
        ...
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        cursor.execute('SELECT name, price , img FROM products')
        products = cursor.fetchall()

        # Pass the product data and the username to the home.html template
        return render_template('home.html', username=session['username'], products=products)
    return redirect(url_for('login'))

# a new route to display user information
@app.route('/user_info')
def user_info():
    if 'loggedin' in session:
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
